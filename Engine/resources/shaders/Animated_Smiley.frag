...
// Shader from https://www.shadertoy.com/view/lsXcWn for testing
...
void main()
{
    ...
    float bg_d = length(uv);
    float bg_r = 0.25;
    float bg_c = smoothstep(bg_r, bg_r - 0.005, d);
    bg_c = 1.0 - bg_c;

    vec3 final_bg_col = vec3(bg_c) * bg_col;

	FragColor = Smiley(uv, m, smile) + vec4(vec3(final_bg_col), 1.0);
}